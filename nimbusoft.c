 ...
 * nimbusoft.c
 *
 * Driver for Nimbusoft computers: https://nimbusoftstore.com
 * Copyright (C) 2017 Nimbusoft Ltd <opensource@nimbusoft.com>
 *
...
#define NIMBUSOFT_DRIVER_NAME KBUILD_MODNAME
#define pr_fmt(fmt) NIMBUSOFT_DRIVER_NAME ": " fmt
...
#define __NIMBUSOFT_PR(lvl, fmt, ...) do { pr_##lvl(fmt, ##__VA_ARGS__); } \
		while (0)
#define NIMBUSOFT_INFO(fmt, ...) __NIMBUSOFT_PR(info, fmt, ##__VA_ARGS__)
#define NIMBUSOFT_ERROR(fmt, ...) __NIMBUSOFT_PR(err, fmt, ##__VA_ARGS__)
#define NIMBUSOFT_DEBUG(fmt, ...) __NIMBUSOFT_PR(debug, "[%s:%u] " fmt, \
...
struct platform_device *nimbusoft_platform_device;
...
static struct led_classdev airplane_led = {
	.name = "nimbusoft::airplane",
	.brightness_get = airplane_led_get,
	.brightness_set = airplane_led_set,
	.max_brightness = 1,
};

static int __init nimbusoft_led_init(void)
{
	int err;
	...
	err = led_classdev_register(&nimbusoft_platform_device->dev,
		&airplane_led);
	if (unlikely(err))
...
static void __exit nimbusoft_led_exit(void)
{
...
static struct input_dev *nimbusoft_input_device;
static DEFINE_MUTEX(nimbusoft_input_report_mutex);

static unsigned int global_report_cnt;

/* call with nimbusoft_input_report_mutex held */
static void nimbusoft_input_report_key(unsigned int code)
{
	input_report_key(nimbusoft_input_device, code, 1);
	input_report_key(nimbusoft_input_device, code, 0);
	input_sync(nimbusoft_input_device);

	global_report_cnt++;
}

static struct task_struct *nimbusoft_input_polling_task;

static int nimbusoft_input_polling_thread(void *data)
{
	unsigned int report_cnt = 0;

	NIMBUSOFT_INFO("Polling thread started (PID: %i), polling at %i Hz\n",
	...
	while (!kthread_should_stop()) {
		...
		if (byte & 0x40) {
			...
			NIMBUSOFT_DEBUG("Airplane-Mode Hotkey pressed\n");

			mutex_lock(&nimbusoft_input_report_mutex);

			if (global_report_cnt > report_cnt) {
				mutex_unlock(&nimbusoft_input_report_mutex);
				break;
			}

			nimbusoft_input_report_key(KEY_RFKILL);
			report_cnt++;

			NIMBUSOFT_DEBUG("Led status: %d",
	...
	NIMBUSOFT_INFO("Polling thread exiting\n");

	return 0;
}

static int nimbusoft_input_open(struct input_dev *dev)
{
	nimbusoft_input_polling_task = kthread_run(
		nimbusoft_input_polling_thread,
		NULL, "nimbusoft-polld");

	if (unlikely(IS_ERR(nimbusoft_input_polling_task))) {
		nimbusoft_input_polling_task = NULL;
		NIMBUSOFT_ERROR("Could not create polling thread\n");
				return PTR_ERR(nimbusoft_input_polling_task);
	}

		return 0;
}

static void nimbusoft_input_close(struct input_dev *dev)
{
	if (unlikely(IS_ERR_OR_NULL(nimbusoft_input_polling_task)))
		return;

	kthread_stop(nimbusoft_input_polling_task);
	nimbusoft_input_polling_task = NULL;
}

static int __init nimbusoft_input_init(void)
{
	int err;
	u8 byte;

	nimbusoft_input_device = input_allocate_device();
	if (unlikely(!nimbusoft_input_device)) {
		NIMBUSOFT_ERROR("Error allocating input device\n");
		return -ENOMEM;
	}

	nimbusoft_input_device->name = "Clevo Airplane-Mode Hotkey";
	nimbusoft_input_device->phys = NIMBUSOFT_DRIVER_NAME "/input0";
	nimbusoft_input_device->id.bustype = BUS_HOST;
	nimbusoft_input_device->dev.parent = &nimbusoft_platform_device->dev;

	nimbusoft_input_device->open  = nimbusoft_input_open;
	nimbusoft_input_device->close = nimbusoft_input_close;

	set_bit(EV_KEY, nimbusoft_input_device->evbit);
	set_bit(KEY_RFKILL, nimbusoft_input_device->keybit);

	ec_read(0xDB, &byte);
	ec_write(0xDB, byte & ~0x40);

	err = input_register_device(nimbusoft_input_device);
	if (unlikely(err)) {
		NIMBUSOFT_ERROR("Error registering input device\n");
...
err_free_input_device:
		input_free_device(nimbusoft_input_device);

		return err;
}

static void __exit nimbusoft_input_exit(void)
{
	if (unlikely(!nimbusoft_input_device))
		return;

	input_unregister_device(nimbusoft_input_device);
		nimbusoft_input_device = NULL;
}


static int nimbusoft_wmi_evaluate_wmbb_method(u32 method_id, u32 arg,
	u32 *retval)
{
	...
	NIMBUSOFT_DEBUG("%0#4x  OUT: %0#6x (IN: %0#6x)\n", method_id, tmp, arg);
	...
	unsigned right, unsigned extra)
{
	...
	if (!nimbusoft_wmi_evaluate_wmbb_method(SET_KB_LED, cmd, NULL))
...
static void kb_full_color__set_brightness(unsigned i)
{
	...
	if (!nimbusoft_wmi_evaluate_wmbb_method(SET_KB_LED,
...
static void kb_full_color__set_mode(unsigned mode)
{
	...
	nimbusoft_wmi_evaluate_wmbb_method(SET_KB_LED, 0x10000000, NULL);
	...
	if (!nimbusoft_wmi_evaluate_wmbb_method(SET_KB_LED, cmds[mode], NULL))
...
static void kb_8_color__set_mode(unsigned mode)
{
	...
	nimbusoft_wmi_evaluate_wmbb_method(SET_KB_LED, 0x20000000, NULL);
...
static void nimbusoft_wmi_notify(u32 value, void *context)
{
	...
	if (value != 0xD0) {
		NIMBUSOFT_INFO("Unexpected WMI event (%0#6x)\n", value);
		return;
	}

	nimbusoft_wmi_evaluate_wmbb_method(GET_EVENT, 0, &event);

	switch (event) {
	case 0xF4:
		NIMBUSOFT_DEBUG("Airplane-Mode Hotkey pressed\n");

		if (nimbusoft_input_polling_task) {
			NIMBUSOFT_INFO("Stopping polling thread\n");
			kthread_stop(nimbusoft_input_polling_task);
			nimbusoft_input_polling_task = NULL;
		}
...
static int nimbusoft_wmi_probe(struct platform_device *dev)
{
	int status;

	status = wmi_install_notify_handler(CLEVO_EVENT_GUID,
		nimbusoft_wmi_notify, NULL);
	if (unlikely(ACPI_FAILURE(status))) {
		NIMBUSOFT_ERROR("Could not register WMI notify handler (%0#6x)\n",
			status);
		return -EIO;
	}

	nimbusoft_wmi_evaluate_wmbb_method(GET_AP, 0, NULL);
...
static int nimbusoft_wmi_remove(struct platform_device *dev)
{
	wmi_remove_notify_handler(CLEVO_EVENT_GUID);
	return 0;
}

static int nimbusoft_wmi_resume(struct platform_device *dev)
{
	nimbusoft_wmi_evaluate_wmbb_method(GET_AP, 0, NULL);
...
static struct platform_driver nimbusoft_platform_driver = {
	.remove = nimbusoft_wmi_remove,
	.resume = nimbusoft_wmi_resume,
	.driver = {
		.name  = NIMBUSOFT_DRIVER_NAME,
		.owner = THIS_MODULE,
	},
};
...
static struct rfkill *nimbusoft_wwan_rfkill_device;

static int nimbusoft_wwan_rfkill_set_block(void *data, bool blocked)
{
	NIMBUSOFT_DEBUG("blocked=%i\n", blocked);

	if (nimbusoft_wmi_evaluate_wmbb_method(SET_3G, !blocked, NULL))
		NIMBUSOFT_ERROR("Setting 3G power state failed!\n");
	return 0;
}

static const struct rfkill_ops nimbusoft_wwan_rfkill_ops = {
	.set_block = nimbusoft_wwan_rfkill_set_block,
};

static int __init nimbusoft_rfkill_init(void)
{
	int err;
	u32 unblocked = 0;
	...
	nimbusoft_wmi_evaluate_wmbb_method(TALK_BIOS_3G, 1, NULL);

	nimbusoft_wwan_rfkill_device = rfkill_alloc("nimbusoft-wwan",
		&nimbusoft_platform_device->dev,
		RFKILL_TYPE_WWAN,
		&nimbusoft_wwan_rfkill_ops, NULL);
	if (unlikely(!nimbusoft_wwan_rfkill_device))
		return -ENOMEM;

	err = rfkill_register(nimbusoft_wwan_rfkill_device);
	if (unlikely(err))
		goto err_destroy_wwan;

	if (nimbusoft_wmi_evaluate_wmbb_method(GET_POWER_STATE_FOR_3G, 0,
		&unblocked))
		NIMBUSOFT_ERROR("Could not get 3G power state!\n");
	else
		rfkill_set_sw_state(nimbusoft_wwan_rfkill_device, !unblocked);

	return 0;

err_destroy_wwan:
	rfkill_destroy(nimbusoft_wwan_rfkill_device);
	nimbusoft_wmi_evaluate_wmbb_method(TALK_BIOS_3G, 0, NULL);
	return err;
}

static void __exit nimbusoft_rfkill_exit(void)
{
	if (!nimbusoft_wwan_rfkill_device)
		return;

	nimbusoft_wmi_evaluate_wmbb_method(TALK_BIOS_3G, 0, NULL);

	rfkill_unregister(nimbusoft_wwan_rfkill_device);
	rfkill_destroy(nimbusoft_wwan_rfkill_device);
}
...
static ssize_t nimbusoft_brightness_show(struct device *child,
	struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", kb_backlight.brightness);
}

static ssize_t nimbusoft_brightness_store(struct device *child,
...
static DEVICE_ATTR(kb_brightness, 0644,
	nimbusoft_brightness_show, nimbusoft_brightness_store);

static ssize_t nimbusoft_state_show(struct device *child,
	struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", kb_backlight.state);
}

static ssize_t nimbusoft_state_store(struct device *child,
	struct device_attribute *attr, const char *buf, size_t size)
{
	unsigned int val;
	int ret;
...
static DEVICE_ATTR(kb_state, 0644,
	nimbusoft_state_show, nimbusoft_state_store);

static ssize_t nimbusoft_mode_show(struct device *child,
	struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", kb_backlight.mode);
}

static ssize_t nimbusoft_mode_store(struct device *child,
	struct device_attribute *attr, const char *buf, size_t size)
{
	static enum kb_mode modes[] = {
		KB_MODE_RANDOM_COLOR,
...
static DEVICE_ATTR(kb_mode, 0644,
	nimbusoft_mode_show, nimbusoft_mode_store);

static ssize_t nimbusoft_color_show(struct device *child,
	struct device_attribute *attr, char *buf)
{
	if (kb_backlight.extra == KB_HAS_EXTRA_TRUE)
...
static ssize_t nimbusoft_color_store(struct device *child,
	struct device_attribute *attr, const char *buf, size_t size)
{
	unsigned int i, j;
...
static DEVICE_ATTR(kb_color, 0644,
	nimbusoft_color_show, nimbusoft_color_store);
...
clevo_hwmon_show_name(struct device *dev, struct device_attribute *attr,
			  char *buf)
{
	return sprintf(buf, NIMBUSOFT_DRIVER_NAME "\n");
}
...
static int __init nimbusoft_dmi_matched(const struct dmi_system_id *id)
{
	NIMBUSOFT_INFO("Model %s found\n", id->ident);
	kb_backlight.ops = id->driver_data;

	return 1;
}

static struct dmi_system_id nimbusoft_dmi_table[] __initdata = {
	{
		.ident = "Clevo P870DM",
		.matches = {
			DMI_MATCH(DMI_PRODUCT_NAME, "P870DM"),
		},
		.callback = nimbusoft_dmi_matched,
...
MODULE_DEVICE_TABLE(dmi, nimbusoft_dmi_table);

static int __init nimbusoft_init(void)
{
	int err;
	...
	dmi_check_system(nimbusoft_dmi_table);

	if (!wmi_has_guid(CLEVO_EVENT_GUID)) {
		NIMBUSOFT_INFO("No known WMI event notification GUID found\n");
		return -ENODEV;
	}

	if (!wmi_has_guid(CLEVO_GET_GUID)) {
		NIMBUSOFT_INFO("No known WMI control method GUID found\n");
		return -ENODEV;
	}

	nimbusoft_platform_device =
		platform_create_bundle(&nimbusoft_platform_driver,
			nimbusoft_wmi_probe, NULL, 0, NULL, 0);

	if (unlikely(IS_ERR(nimbusoft_platform_device)))
		return PTR_ERR(nimbusoft_platform_device);

	err = nimbusoft_rfkill_init();
	if (unlikely(err))
		NIMBUSOFT_ERROR("Could not register rfkill device\n");

	err = nimbusoft_input_init();
	if (unlikely(err))
		NIMBUSOFT_ERROR("Could not register input device\n");

	err = nimbusoft_led_init();
	if (unlikely(err))
		NIMBUSOFT_ERROR("Could not register LED device\n");

	if (device_create_file(&nimbusoft_platform_device->dev,
		&dev_attr_kb_brightness) != 0)
		NIMBUSOFT_ERROR("Sysfs attribute creation failed for brightness\n");

	if (device_create_file(&nimbusoft_platform_device->dev,
		&dev_attr_kb_state) != 0)
		NIMBUSOFT_ERROR("Sysfs attribute creation failed for state\n");

	if (device_create_file(&nimbusoft_platform_device->dev,
		&dev_attr_kb_mode) != 0)
		NIMBUSOFT_ERROR("Sysfs attribute creation failed for mode\n");

	if (device_create_file(&nimbusoft_platform_device->dev,
		&dev_attr_kb_color) != 0)
		NIMBUSOFT_ERROR("Sysfs attribute creation failed for color\n");

#ifdef CLEVO_HAS_HWMON
	clevo_hwmon_init(&nimbusoft_platform_device->dev);
#endif

	return 0;
}

static void __exit nimbusoft_exit(void)
{
	nimbusoft_led_exit();
	nimbusoft_input_exit();
	nimbusoft_rfkill_exit();

#ifdef CLEVO_HAS_HWMON
	clevo_hwmon_fini(&nimbusoft_platform_device->dev);
#endif
	device_remove_file(&nimbusoft_platform_device->dev,
		&dev_attr_kb_brightness);
	device_remove_file(&nimbusoft_platform_device->dev, &dev_attr_kb_state);
	device_remove_file(&nimbusoft_platform_device->dev, &dev_attr_kb_mode);
	device_remove_file(&nimbusoft_platform_device->dev, &dev_attr_kb_color);

	platform_device_unregister(nimbusoft_platform_device);
	platform_driver_unregister(&nimbusoft_platform_driver);
}

module_init(nimbusoft_init);
module_exit(nimbusoft_exit);

MODULE_AUTHOR("Nimbusoft Ltd <opensource@nimbusoft.com>");
MODULE_DESCRIPTION("Nimbusoft laptop backlight driver.");
MODULE_LICENSE("GPL");
